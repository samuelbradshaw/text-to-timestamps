...
def transcribe(method, audio_path, lang = 'en', get_words_if_available = False, model_size = 'small', model_name = None, batch_size = 16):
  ...
  method_name = text_to_timestamps.utils.supported_transcribe_methods[method]['name']
  if not method_name:
    return {}
  ...
  cached_model = cached_transcribe_models.get(method, {}).get(model_size or model_name, {}).get(lang)
  if not cached_model:
  ...
  map_bcp47_to_fb_lang = { 'en': 'eng', 'es': 'spa', 'sq': 'sqi' }
  ...
  if method == 'distil-whisper-mlx':
    import mlx_whisper
    result['data'] = mlx_whisper.transcribe(audio_path, path_or_hf_repo = f'mlx-community/{model_name}', word_timestamps = get_words_if_available, language = lang, condition_on_previous_text = False)
    result['text'] = result['data']['text'].strip()
    if get_words_if_available:
...
#     result['data'] = asr.transcribe([audio_path], lang = lang_fb, align = False)
  ...
  # faster-whisper
    ...
    model = cached_model or faster_whisper.WhisperModel(model_name, device = device, compute_type = compute_type)
    if batch_size > 1:
      model = cached_model or faster_whisper.BatchedInferencePipeline(model = model)
    result['data'] = model.transcribe(audio_path, word_timestamps = get_words_if_available, batch_size = batch_size, language = lang, condition_on_previous_text = False)
...
#     result['data'] = asr(audio_file = audio_path, lang = lang, sample_rate = sr, model = model_name)    
  ...
  # Parakeet MLX
    ...
    import parakeet_mlx
    model = cached_model or parakeet_mlx.from_pretrained(f'mlx-community/{model_name}')
  ...
  # pywhispercpp
    ...
    model = cached_model or pywhispercpp.model.Model(model_name, token_timestamps = True, max_len = 1, split_on_word = True)
    # Set sample rate to 16000 Hz
    with tempfile.NamedTemporaryFile(delete = False) as f:
      sound = pydub.AudioSegment.from_file(audio_path).set_frame_rate(16000)
      temp_path = f.name
      sound.export(temp_path, format = 'wav')
    result['data'] = model.transcribe(temp_path, language = lang)
  ...
  # stable-ts
    ...
    result['data'] = model.transcribe(audio_path, language = lang, task = 'transcribe', condition_on_previous_text = False).to_dict()
    result['text'] = result['data']['text'].strip()
    if get_words_if_available:
      for segment in result['data']['segments']:
        for word in segment['words']:
  ...
  # whisper-mps
    ...
    result['data'] = whisper_mps.transcribe(audio_path, model = model_name, language = lang, condition_on_previous_text = False)
    result['text'] = result['data']['text'].strip()
  
  # Whisper (OpenAI)
  elif method == 'whisper-openai':
    import whisper
    model = cached_model or whisper.load_model(model_name, device = device)
    result['data'] = model.transcribe(audio_path, word_timestamps = get_words_if_available, fp16 = device != 'cpu', language = lang, condition_on_previous_text = False)
    result['text'] = result['data']['text'].strip()
    if get_words_if_available:
      for segment in result['data']['segments']:
        for word in segment['words']:
  ...
  # whisper-timestamped
    ...
    model = cached_model or whisper_timestamped.load_model(model_name, device = device)
    result['data'] = model.transcribe(audio_path, word_timestamps = get_words_if_available, fp16 = device != 'cpu', language = lang, condition_on_previous_text = False)
    result['text'] = result['data']['text'].strip()
    if get_words_if_available:
      for segment in result['data']['segments']:
  ...
  # WhisperS2T
    ...
    import whisper_s2t
    model = cached_model or whisper_s2t.load_model(model_identifier = model_name, backend = 'CTranslate2', compute_type = compute_type, device = device, asr_options = {'word_timestamps': get_words_if_available})
  ...
  # WhisperX
    ...
    import whisperx
    model = cached_model or whisperx.load_model(model_name, device, compute_type = compute_type, language = lang, task = 'transcribe')
  ...
  if model:
    ...
    if not method in cached_transcribe_models:
      cached_transcribe_models[method] = {}
    if not model_key in cached_transcribe_models[method]:
      cached_transcribe_models[method][model_key] = {}
    cached_transcribe_models[method][model_key][lang] = model
  
  return result


# Clear cached models
cached_transcribe_models = {}
def clear_cached_transcribe_models():
  cached_transcribe_models = {}
